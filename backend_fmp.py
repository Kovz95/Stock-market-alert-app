...
_LEGACY_BYTECODE = Path(__file__).parent / "__pycache__/backend_fmp_legacy.cpython-311.pyc"
_PYC_HEADER_SIZE = 16
_BOOTSTRAPPED = False
...
def _load_legacy_code() -> CodeType:
    if not _LEGACY_BYTECODE.exists():
        raise FileNotFoundError(
            f"Missing legacy bytecode for backend_fmp at {_LEGACY_BYTECODE}."
        )
