...
import {
  useHourlyDiscordConfig,
  useCopyDailyToHourly,
  useUpdateHourlyChannelWebhook,
...
export default function HourlyDiscordPage() {
  const { data: config, isLoading, error } = useHourlyDiscordConfig();
  const copyDailyToHourly = useCopyDailyToHourly();
  ...
  async function handleCopyDailyToHourly() {
    const result = await copyDailyToHourly.mutateAsync();
    if (result.success) {
      toast.success("Copied daily webhooks into hourly configuration.");
    } else {
  ...
  if (isLoading) {
    return (
      <div className="p-8">
        <p className="text-muted-foreground">Loading hourly Discord config…</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-8 max-w-4xl">
      <h1 className="text-2xl font-bold mb-2">Hourly Discord Channel Management</h1>
      <p className="text-muted-foreground mb-6">
        Configure dedicated Discord webhooks for <strong>hourly</strong> timeframe alerts.
      </p>

      <DiscordMetrics config={config} routingLabel="Hourly Routing" />

      <div className="mt-4">
        <Button
          variant="outline"
          onClick={handleCopyDailyToHourly}
          disabled={copyDailyToHourly.isPending}
        >
          {copyDailyToHourly.isPending ? "Copying…" : "Copy Daily Webhooks → Hourly"}
        </Button>
        <span className="ml-2 text-xs text-muted-foreground">
          Populate hourly channels using existing daily webhook URLs.
        </span>
      </div>

      <Separator className="my-6" />

      <h2 className="text-lg font-semibold mb-4">Configure Hourly Channels</h2>
      <p className="text-muted-foreground text-sm mb-4">
        Update the webhook URL for each hourly economy or special channel.
      </p>
      ...
      <DiscordTestSection
        title="Test Hourly Routing"
        description="Enter a ticker to see which hourly channel it would use. Use Send test message to post a test to that channel."
