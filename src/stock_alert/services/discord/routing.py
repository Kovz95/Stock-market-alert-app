...
class DiscordEconomyRouter:
    ...
    def __init__(
        self, config_file: str = "discord_channels_config.json", use_rate_limiter: bool = True
    ):
        """
    ...
    def _load_config(self) -> dict:
        ...
        try:
            ...
            logger.warning(
                f"Discord config file {self.config_file} not found. Using default configuration."
            )
    ...
    def _load_stock_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            logger.warning(
                "Neither PostgreSQL metadata nor cleaned_data.csv found. Industry routing will be limited."
            )
            return pd.DataFrame()
                ...
                for cond_item in conditions_list:
                    if isinstance(cond_item, dict) and "conditions" in cond_item:
                        ...
                        alert_conditions.append(
                            cond_item[0] if isinstance(cond_item[0], str) else str(cond_item[0])
                        )
                        ...
                        if conditions_list and len(conditions_list) > 0:
                            for cond_item in conditions_list:
                                if isinstance(cond_item, dict) and "conditions" in cond_item:
                                    ...
                                    break
                    logger.info(
                        f"Alert condition '{alert_cond_str}' matched custom channel '{channel_name}' (condition: {condition})"
                    )

        return matching_channels

    def get_stock_economy(self, ticker: str) -> str | None:
        """
        ...
        # Try partial match
        match = self.stock_data[
            self.stock_data["Symbol"].str.contains(clean_ticker, case=False, na=False)
        ]
        if not match.empty and "Economy" in match.columns:
            economy = match.iloc[0]["Economy"]
            if pd.notna(economy) and economy != "":
    ...
    def _get_channel_mapping(self, timeframe: str | None) -> tuple[dict, str]:
    ...
    def determine_alert_channel(self, alert: dict) -> tuple[str, str]:
        ...
        return self._get_channel_info(
            self.config.get("default_channel", "General"), alert_timeframe
        )

    def _get_channel_info(self, channel_name: str, timeframe: str | None = None) -> tuple[str, str]:
        """
    ...
    def send_economy_alert(self, alert: dict, message: str) -> bool:
        ...
        try:
            ...
            if not webhook_url:
                ...
                channel_name, webhook_url = self._get_channel_info(
            ...
            if webhook_url == "YOUR_GENERAL_WEBHOOK_URL_HERE" or "YOUR_" in webhook_url:
                logger.warning(
                    f"Discord webhook not configured for {channel_name}. Please update {self.config_file}"
                )
            else:
                ...
                if self.rate_limiter:
                    success_send, status = self.rate_limiter.send_with_rate_limit(
                        webhook_url, payload
                    )
                    if success_send:
                        ...
                        logger.error(
                            f"Failed to send alert to {channel_name} (rate limited or error)"
                        )
                else:
                    ...
                    if response.status_code == 204:
                        ...
                        logger.error(
                            f"Failed to send alert to {channel_name}: {response.status_code} - {response.text}"
                        )
            ...
            for custom_name, custom_info in custom_channels:
                try:
                    ...
                    if spacing_config.get("enabled", False):
                        ...
                        formatted_message = (
                            f"**{custom_info.get('channel_name', custom_name)}**\n{message}"
                        )
                    ...
                    if self.rate_limiter:
                        ...
                        if success_send:
                            ...
                            logger.error(
                                f"Failed to send alert to custom channel {custom_name} (rate limited or error)"
                            )
                    else:
                        # Original sending logic without rate limiting
                        response = requests.post(webhook_url, json=payload, timeout=10)
                        ...
                        if response.status_code == 204:
                            ...
                            logger.error(
                                f"Failed to send alert to custom channel {custom_name}: {response.status_code}"
                            )
        ...
        if channels_sent:
            ...
            logger.warning("Alert was not sent to any channels")

        return success

    def get_available_channels(self, timeframe: str | None = None) -> list[dict]:
        """
    ...
    def update_channel_config(
        self, channel_name: str, webhook_url: str, timeframe: str | None = None
    ) -> bool:
        """
        ...
        try:
            ...
            if channel_name not in mapping:
                logger.error(
                    f"Channel {channel_name} not found in configuration (timeframe={timeframe})"
                )
                return False
            ...
            logger.info(
                f"Updated webhook URL for {channel_name} (timeframe={self._normalize_timeframe(timeframe)})"
            )
            return True
        except Exception as e:
...
def get_stock_economy_classification(ticker: str) -> str | None:
    """
