...
def _query_dataframe(sql: str, params: Iterable[Any] | None = None) -> pd.DataFrame:
    ...
    try:
        df = pd.read_sql_query(sql, conn, params=params if params is None else list(params))
        return df
    finally:
...
def fetch_stock_metadata_df() -> pd.DataFrame:
    ...
    if not df.empty:
        ...
        set_json(
            ...
            {"records": _normalise_records([{str(k): v for k, v in record.items()} for record in df.to_dict(orient="records")])},
            ttl_seconds=600,
        )
    return df
