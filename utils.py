...
def get_fmp_ticker_with_fallback(ticker):
    ...
    if "." in ticker:
        ...
        us_fallback_exchanges = [
            ...
            "NS",
            "FR",
            "CO",  # Based on investigation results
        ]
...
def send_alert(stock, alert, condition_str, df):
    ...
    send_stock_alert(
        WEBHOOK_URL, timeframe, alert["name"], stock, condition_str, current_price, action, exchange
    )
...
def get_rate_limit_recommendations(minute_utilization, hour_utilization):
    ...
    print(f"{'='*50}")
    print(
        f"[TIME] Requests (last minute): {status['requests_last_minute']}/{status['limit_per_minute']} ({status['minute_utilization_percent']}%)"
    )
    print(
        f"[TIME] Requests (last hour): {status['requests_last_hour']}/{status['limit_per_hour']} ({status['hour_utilization_percent']}%)"
    )
